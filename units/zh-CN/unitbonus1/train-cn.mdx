...
在本笔记本中，我们将通过**教哈吉狗拿起棍子然后直接在浏览器中玩它**来巩固我们在第一个单元中学到的知识

⬇️ 这是**您将在本单元结束时实现的示例。** ⬇️（启动 ▶ 以查看）
...
- 哈吉狗， [Thomas Simonini](https://twitter.com/ThomasSimonini) 基于 [Puppo The Corgi](https://blog.unity.com/technology/puppo-the-corgi- cuteness-overload-with-the-unity-ml-agents-toolkit)

### 使用的库📚

- [MLAgents（Hugging Face）](https://github.com/huggingface/ml-agents)

我们一直在努力改进我们的教程，所以**如果您在此笔记本中发现一些问题**，请[在 Github Repo 上打开一个问题](https://github.com/huggingface/deep-rl-class /问题）。

## 本章的目标 🏆

在本章的末尾，您将：

- 了解**用于训练哈吉的状态空间、动作空间和奖励函数**。
- **训练你自己的哈吉** 来拿棍子。
- 能够直接在浏览器中与训练有素的哈吉一起玩**。
...
- 我们需要克隆存储库，它**包含允许您将训练有素的代理推送到仓库的实验版本。**
...
哈吉不会“看到”他的环境。相反，我们向他提供有关环境的信息：

- 目标（摇杆）位置
- 自己和目标之间的相对位置
- 他腿的方向。

鉴于所有这些信息，哈吉**可以决定接下来要采取什么行动来实现他的目标**。
...
**关节电机驱动抱腿**。这意味着为了达到目标，哈吉需要**学会正确地旋转每条腿的关节马达，这样他才能移动**。

### 奖励函数

奖励函数的设计是为了让 **哈吉实现他的目标**：拿到棍子。

请记住，强化学习的基础之一是*奖励假设*：目标可以描述为**预期累积奖励**的最大化**。

在这里，我们的目标是哈吉**朝着棍子走去，但不要旋转太多**。因此，我们的奖励函数必须转化为这个目标。
...
- *旋转惩罚*：如果哈吉旋转太多并且转得太快**，我们会对哈吉进行惩罚。
- *达到目标奖励*：我们奖励哈吉**达到目标**。

## 检查哈吉配置文件

- 在 ML-Agents 中，您将**训练超参数定义到 config.yaml 文件中。**

- 对于本笔记本的范围，我们不打算修改超参数，但如果您想尝试作为实验，您还应该尝试修改其他一些超参数，Unity 提供了非常好的文档，在这里解释了它们中的每一个] （https://github.com/Unity-Technologies/ml-agents/blob/main/docs/Training-Configuration-File.md）。
...
培训将需要 30 到 45 分钟，具体取决于您的机器（不要忘记**设置 GPU**），去参加☕️你应得的🤗。
...
- 现在我们已经训练了我们的智能体，我们**准备将其推送到仓库，以便能够在您的浏览器上与哈吉一起玩🔥。**

为了能够与社区共享您的模型，还需要执行三个步骤：
...
3. `--repo-id`：您要创建或更新的 Hugging Face 存储库的名称。总是<your huggingface="None" username="None">/<the repo="None" name="None">
如果回购不存在**它将自动创建**
4. `--commit-message`：由于 HF 存储库是 git 存储库，因此您需要定义提交消息。
...
它是您的模型存储库的链接。存储库包含一个模型卡，解释如何使用模型、您的 Tensorboard 日志和您的配置文件。 **很棒的是它是一个 git 存储库，这意味着您可以进行不同的提交、使用新推送更新您的存储库、打开 Pull Requests 等。**
