...
An agent learns to **make decisions by trial and error, with rewards and punishments from the surroundings**. In other words - an agent is the entity that interacts with the environment, makes decisions, and learns from the rewards or penalties it receives. The agent's goal is to find an optimal strategy (policy) to maximize its cumulative reward over time.
